...
class OptimizedLOF:
    ...
    def fit(self, X_train_scaled):
        ...
        try:
            self.lof_model.fit(self.X_train_)
            return True
        except:
            return False
    ...
    def decision_function(self, X_test_scaled):
        ...
        try:
            # LOF decision_function: âm = normal, dương = outlier
            scores = self.lof_model.decision_function(X_test_scaled)
            # Chuyển thành: cao = outlier, thấp = normal
...
def process_single_configuration_fixed(args):
    ...
    try:
        ...
        threshold = 0.0  # Initialize threshold
        ...
        for batch_start in range(start_idx, len(df), batch_size):
            ...
            if model_fitted:
                ...
                threshold = np.percentile(train_scores, 75)  # Top 25% là những điểm BENIGN coi là outlier
...
def plot_results_corrected(results_df, attack_label):
    ...
    plt.tight_layout()
    plt.savefig(f'LOF_{attack_label}_performance.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(f'LOF_{attack_label}_detection_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    # Lưu kết quả
    results_df.to_csv(f'LOF_{ATTACK_LABEL}_results.csv', index=False)
    print(f"\nĐã lưu kết quả vào 'LOF_{ATTACK_LABEL}_results.csv'")
